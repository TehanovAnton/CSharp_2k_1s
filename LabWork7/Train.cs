...
namespace LabWork6
{
    public sealed class Train : Vehicle, Engine
    {
        ...
        private static List<RailwayCarriage> SetIfValid(int maxValue, List<RailwayCarriage> value, string exceptionMessage)
        {
            if (value.Count > maxValue || value.Count <= 0) 
            {
                throw new TrainException(exceptionMessage, value);
        ...
        {
            this.carriages = SetIfValid(1, carriages, "превышено количество вагонов");
